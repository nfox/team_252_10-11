...
/*
 * $Id: HTMC-driver.h 29 2010-06-25 12:55:41Z xander $
 */
...
int HTMCreadRelativeHeading(tSensors link) {
  ...
  return ((HTMCreadHeading(link) - target[link][0] + 540) % 360 - 180);

  /*
  int _tmpheading = 0;

  _tmpheading = HTMCreadHeading(link);
  if (_tmpheading < 0)
    return -255;

  // Compensate for offset and correct if result is less than 0
  _tmpheading = (_tmpheading - target[link][0]) % 360;
  if (_tmpheading < 0)
    _tmpheading += 360;
